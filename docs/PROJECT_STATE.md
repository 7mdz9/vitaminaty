...
**M2 - Admin portal: Step 1b audit-service COMPLETE locally (2026-05-24). Next: Step 2 - MFA enrollment.**
...
- **Audit-service diff contract.** `docs/DB_SCHEMA.md §8.1.1` is the source of truth for the seven `audit_log.diff` JSONB shape families. `src/lib/audit/diff-types.ts` mirrors that contract and `src/server/services/audit-service.ts` is the baseline write orchestrator. PII is stored raw in audit rows for forensic/compliance use; redaction is renderer-side only in Step 12. Secrets, credentials, tokens, and password material must never be written to `audit_log.diff`.
...
| `tests/integration/services/audit-service.test.ts` | M2 Step 1b integration tests for all seven audit diff shapes and request context capture. |
