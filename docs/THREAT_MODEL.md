...
| Inventory movement ledger | Medium - operational integrity | `inventory_movements` table (append-only, admin-read RLS only; no customer PII) |
...
M1 addendum 0012 adds `inventory_movements` as a new admin-read RLS-gated table following the M1 append-only ledger pattern. No customer PII is stored in this table; writes are service-role-only through `src/server/repositories/inventory-movement-repository.ts`, and no INSERT/UPDATE/DELETE RLS policies exist. The new surface does not add customer-data exposure beyond the existing service-role admin write path.
...
- M2 Step 2 implements the forced TOTP gate. Recovery codes are generated by the app because Supabase MFA does not provide them; only HMAC-SHA-256 hashes are stored in `admin_mfa_recovery_codes`, plaintext codes are shown once, and `mfa_enrolled` audit diffs record only the count plus factor metadata.
...
| Date | Version | Change |
|---|---|---|
| 2026-05-23 | 1.0.17 | M1 addendum 0012 of M1 spec evolution: inventory tracking schema landed; `stock_status` trigger-derived; `inventory_movements` append-only with admin-read RLS only; `missing_stock_quantity` flag emitted for 787 imported products. |
