    ...
    <Dialog open={open} onOpenChange={(next) => (!next ? onCancel() : undefined)}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Soft review flags require an explicit reason. Hard-blocked products are excluded server-side.
          </DialogDescription>
        </DialogHeader>
        <div className="max-h-64 space-y-2 overflow-y-auto">
          {Object.entries(reviewFlagsByProductId).map(([productId, flags]) => (
            <div className="rounded-admin-sm border border-admin-border p-2 text-admin-sm" key={productId}>
          ...
          {hardBlockedProductIds.length > 0 ? (
            <div className="rounded-admin-sm border border-admin-danger p-2 text-admin-sm text-admin-danger">
              {hardBlockedProductIds.length} hard-blocked product{hardBlockedProductIds.length === 1 ? "" : "s"} excluded.
            </div>
          ) : null}
        </div>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={() => onConfirm(reason)} disabled={reason.trim().length < 3}>
            Publish anyway
          </Button>
        </DialogFooter>
      </DialogContent>
